...
private:
    ...
    // variables
    Node<T,K>* root;
    Node<T,K>* smallest; // most left in the tree
    int size;
    ...
    //Node<T,K>* LoadSortedArray2(K* sortedKeysArray, T* sortedArray, int length,
                           Node* parent);
    Node<T,K>* findClosestParentOf(K key);
    int findLeftest(K key);
    std::shared_ptr<T> Select(Node<T,K>* node, int k)
public:
    AVLRankTree();
    virtual ~AVLRankTree();
    void Reset();
    ...
    void Insert(K key, std::shared_ptr<T> data);
    void Remove(K key);
    std::shared_ptr<T> getByKey(K key);
    std::shared_ptr<T> getByRank(int i);
    //bool DoesExist(K key);
    //std::shared_ptr<T> getSortedArray();
    //std::shared_ptr<T>& getSmallest();
    void print();
};
...
template<class T, class K>
typename AVLRankTree<T,K>::Node<T,K>* AVLRankTree<T,K>::findClosestParentOf(K key) {
...
template<class T, class K>
int AVLRankTree<T,K>::getInRange(K min, K max) {
...
template<class T, class K>
void AVLRankTree<T,K>::LoadSortedArray(K* sortedKeysArray, T* sortedDataArray,
                                        int length) {
...
// Time complexity: O(log(n))
template<class T, class K>
void AVLRankTree<T,K>::Remove(K key) {
    Node<T,K>* node = getNode(key);
    if (node)
        RemoveNode(node);
    return;
}
...
template<class T, class K>
std::shared_ptr<T> AVLRankTree<T,K>::getByKey(K key) {
    Node<T,K>* node = getNode(key);
    if (!node)
        return nullptr;
    return node->data;
...
// Time complexity: log(n)
template<class T, class K>
std::shared_ptr<T> AVLRankTree<T,K>::getByRank(int i) {
...
// Time complexity: log(n)
template<class T, class K>
bool AVLRankTree<T,K>::DoesExist(K key) {
...
// Time complexity: O(n)
template<class T, class K>
T* AVLRankTree<T,K>::getSortedArray() {
...
/********************************* Private Functions *******************************/
template<class T, class K>
std::shared_ptr<T> AVLRankTree<T,K>::Select(Node<T,K>* node, int k) {
...
template<class T, class K>
void AVLRankTree<T,K>::RemoveNode(Node<T,K>* node) {
    // if leaf
...
template<class T, class K>
void AVLRankTree<T,K>::setAllRanks(Node<T,K>* subroot) {
    if (!subroot)
...
template<class T, class K>
void AVLRankTree<T,K>::balance(Node<T,K>* node) {
...
template<class T, class K>
void AVLRankTree<T,K>::llRotation(Node<T,K>* node) {
    Node<T,K>* parent = node->parent;
    Node<T,K>* lChild = node->left;

    // parent-node relation
    ParentPointTo(node, lChild);
    lChild->parent = parent;

    Node<T,K>* lrChild = node->left->right; //backup
    // node-child relation
    lChild->right = node;
    node->parent = lChild;
    // restore lost chain
...
// node: the node with the |balance|>=2
template<class T, class K>
void AVLRankTree<T,K>::rrRotation(Node<T,K>* node) {
    Node<T,K>* parent = node->parent;
    Node<T,K>* rChild = node->right;

    // parent-node relation
    ParentPointTo(node, rChild);
    rChild->parent = parent;

    Node<T,K>* rlChild = node->right->left; //backup
    // node-child relation
    rChild->left = node;
    node->parent = rChild;
    // restore lost chain
...
// node: the node with the |balance|>=2
template<class T, class K>
void AVLRankTree<T,K>::lrRotation(Node<T,K>* node) {
...
// node: the node with the |balance|>=2
template<class T, class K>
void AVLRankTree<T,K>::rlRotation(Node<T,K>* node) {
    llRotation(node->right);
    rrRotation(node);
}

template<class T, class K>
typename AVLRankTree<T,K>::Node<T,K>* AVLRankTree<T,K>::getNode(K key) {
    Node<T,K>* current = root;
...
template<class T, class K>
void AVLRankTree<T,K>::print2(Node<T,K>* nodeToPrint, int level) {
    int i;
    if (nodeToPrint) {
...
template<class T, class K>
T* AVLRankTree<T,K>::getSortedArray2(T* array, Node<T,K>* node) {
    if (node == NULL)
        return array;
...
template<class T, class K>
void AVLRankTree<T,K>::destroy2(Node<T,K>* node) {
    if (node == NULL)
        return;
    destroy2(node->left);
    destroy2(node->right);
...
/*
template<class T, class K>
typename AVLRankTree<T,K>::Node<T,K>* AVLRankTree<T,K>::LoadSortedArray2(
        K* sortedKeysArray, T* sortedDataArray, int length, Node<T,K>* parent) {
    if (length == 0)
        return NULL;
...
template<class T, class K>
int AVLRankTree<T,K>::findLeftest(K key) {
    int smallerNodes = 0;
    Node<T,K>* current = this->findClosestParentOf(key);
    if (current == NULL) {
        return 0;
    }
    ...
    while (current != root) {
        Node<T,K>* parent = current->parent;
